...
import { selectMovie, selectRegion, selectSeats, selectShowing } from "../actions";
...
class RegionSelection extends React.Component {
  ...
  renderOptions() {
    ...
    return locales.map(locale => {
      return (
        <button key={locale[1]} className={`option-button ${this.props.region.region === locale[0] ? "active" : ""}`}
                onClick={() => {
